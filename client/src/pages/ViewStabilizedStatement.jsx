...
class ViewStabilizedStatement extends React.Component
{
    ...
    downloadWordSummary()
    {
        window.location = reportUrl(this.props.appraisal._id, "stabilized_statement", "word");
    }


    downloadExcelSummary()
    {
        window.location = reportUrl(this.props.appraisal._id, "stabilized_statement", "excel");
    }
    ...
    onUnitClicked(unit, unitIndex)
    {
        ...
        {
            this.props.navigate(`/appraisal/${this.props.appraisal._id}/tenants/rent_roll?unit=${unitIndex}`)
        }
        else
        {
            // do nothing
    ...
    createNewIncomeStatementItem(field, value, incomeField)
    {
        if (value !== null && value !== "")
        {
            ...
            const cashFlowType = incomeField === 'expenses' ? 'expense' : 'income';
            const incomeStatementItemType = incomeField === 'expenses' ? 'operating_expense' : 'additional_income';
    ...
    render()
    {
        ...
        return [
            <AppraisalContentHeader key="header" appraisal={this.props.appraisal} title="Stabilized Statement Valuation"/>,
                                ...
                                <Col xs={12} lg={8}>
                                    <div className={"stabilized-statement-centered"}>
                                        ...
                                        <Table className={"statement-table"}>
                                            ...
                                            {
                                                (this.props.appraisal.appraisalType === 'simple' && this.props.appraisal.incomeStatement.items || []).map((incomeItem, index) =>
                                                {
