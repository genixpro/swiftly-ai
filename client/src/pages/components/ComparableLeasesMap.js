...
class ComparableLeasesMap extends React.Component {
    ...
    render()
    {
        ...
        if (!window.__SWIFTLY_GOOGLE_MAPS_API_KEY__)
        {
            return <OpenStreetMapFallback subject={this.props.appraisal} comparables={this.props.comparableLeases} label="Comparable leases" />;
        }

        return [
            <div className={"comparable-leases-map"} id={"comparable-leases-map"}>
                <GoogleMapReact
                    bootstrapURLKeys={{ key: window.__SWIFTLY_GOOGLE_MAPS_API_KEY__ || "" }}
