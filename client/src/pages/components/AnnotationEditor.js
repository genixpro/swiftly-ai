...
class AnnotationEditor extends React.Component
{
    ...
    reprocessFile()
    {
        return this.props.reprocessFile().then(() =>
            axios.get(`/appraisal/${this.props.appraisalId}/files/${this.props.fileId}`)
                .then((response) => this.setState({file: FileModel.create(response.data.file)}))
        );
    }
        ...
        return (
            <div id={"annotation-editor-extractions"} onMouseUp={this.onMouseUp.bind(this)} onMouseMove={this.onMouseMove.bind(this)}>
                <Row>
                    <Col xs={1}>
                        {
                            _.range(1, Number(this.state.file.pages || 0) + 1).map((page) =>
                            {
                                return <Card outline color="primary" className={`annotation-editor-thumbnail-container ${page === this.state.currentPage ? "selected" : ""}`} key={page}>
                                    <CardBody>
                                        <img
                    ...
                    <Col xs={9}>
                        <div className={"extractions-toolbar"}>
                            <div>
                                <Button color={"secondary"} onClick={() => this.changePage(this.state.currentPage === 1 ? this.state.file.pages : this.state.currentPage - 1)}>Previous</Button>
                            </div>
                            <div>
                                <Button color={"secondary"} onClick={() => this.changePage(this.state.currentPage === this.state.file.pages ? 1 : this.state.currentPage + 1)}>Next</Button>
                            </div>
                            <div>
                                <span>View:</span>
                            </div>
