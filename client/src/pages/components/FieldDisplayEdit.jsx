...
class FieldDisplayEdit extends React.Component
{
    ...
    render() {
        ...
        const rendered = (
            <div ref={this.props.dropRef} className={`field-display-edit ${editStateClass} ${customClass} ${editableClass} ${hideInput} ${typeClass}`}>
                <InputGroup
...
function DroppableFieldDisplayEdit(props) {
    const uniqueId = React.useId();
    const {setNodeRef, isOver} = useDroppable({
        id: props.id || `field-${uniqueId}`,
        data: {onDrop: (item) => {
        if (item?.type === 'Word' && (props.type === 'currency' ||
            props.type === 'number' ||
            props.type === 'percent' ||
            props.type === 'length' ||
            props.type === 'area' ||
            ...
            props.type === 'text'))
        {
            props.onChange(FieldDisplayEdit.cleanValue(props.type, item.word.word), [item.word.index]);
        }
    }}});
    return <FieldDisplayEdit {...props} dropRef={setNodeRef} isOver={isOver}/>;
}
