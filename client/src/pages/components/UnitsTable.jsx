...
const DragHandle = ({attributes, listeners}) => <button
    type="button"
...
class UnitRow extends React.Component
{
    ...
    componentDidMount()
    {
        if (this.props.search != null && this.props.allowSelection && this.getQueryVariable("unit").toString() === (this.props.unitIndex || "null").toString())
        {
    ...
    render()
    {
        ...
        return [<tr ref={this.props.rowRef} style={this.props.sortableStyle} onClick={(evt) => this.onUnitClicked()} className={"unit-row " + selectedClass} key={0}>
            {
                this.props.allowSelection ?
                    <td>
                        <DragHandle {...this.props.dragHandleProps} />
                    </td> : null
            }
            {
...
function SortableItem({id, value, index, fields, fieldConfiguration, appraisal, onChangeUnit, onUnitClicked, removeUnit, allowSelection, search}) {
...
function SortableRows({items, onSortEnd, children}) {
...
class UnitsTable extends React.Component
{
    ...
    render() {
        ...
        const sortableIds = this.props.appraisal.units.map((unit, index) => `unit-${unit._id || unit.unitNumber || 'row'}-${index}`);
        ...
        return (
            (this.props.appraisal) ?
                <div>
                <SortableRows items={sortableIds} onSortEnd={this.onSortEnd}>
