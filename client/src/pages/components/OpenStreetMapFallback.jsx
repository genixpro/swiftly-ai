...
export default function OpenStreetMapFallback({subject, comparables = [], label}) {
    const subjectLocation = coordinates(subject);
    const points = comparables.map((comparable) => ({comparable, location: coordinates(comparable)})).filter((point) => point.location);
