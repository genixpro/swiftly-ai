...
def main():
    ...
    print("Connected to environment db, updating objects")
    PropertyTag.objects().update(owner=newOwner)
    Appraisal.objects().update(owner=newOwner)
    File.objects().update(owner=newOwner)
    ComparableSale.objects().update(owner=newOwner)
    ComparableLease.objects().update(owner=newOwner)
    Zone.objects().update(owner=newOwner)
    Image.objects().update(owner=newOwner)
